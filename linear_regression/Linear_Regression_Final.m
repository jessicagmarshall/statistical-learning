...
%% Table 3.3: Estimated Coefficients and Test Error Results for Different
%Subset and Shrinkage Methods
...
rowlabels3 = [rowlabels3 ; 'Test error']; % Add test error label to table

fprintf('The table below corresponds to Table 3.3 of the prostate example:');
fprintf('\n')
fprintf('\n')

y_ridge = X * b_ridge(:, 18);
y_lasso = X * b_lasso(:, 18);

test_err3 = [sum((y_hat - y).^2)/size(y, 1) sum((y_hat_k - y).^2)/size(y, 1) ...
    sum((y_ridge - y).^2)/size(y, 1) sum((y_lasso - y).^2)/size(y, 1)];    % build test error row

tablearray = [beta_hat beta_BS b_ridge(:, 18) b_lasso(:, 18) ; test_err3];
table3 = array2table(tablearray, 'VariableNames', titlerow3, 'RowNames', rowlabels3);  %output table 3.3
